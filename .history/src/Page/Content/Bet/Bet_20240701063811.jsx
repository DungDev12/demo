  ...
  return (
    <>
      <div className="border-[#E77F08] border-[1px] rounded-[4px] w-[50%] p-[15px]">
        ...
        <button className="bg-[#e77f08] text-[#ffffff] py-[6px] px-[13px] text-[14px] rounded-[4px] mx-auto block mt-[10px]">
          Kiểm tra
        </button>
          ...
          {dataContact &&
            dataContact.map((d, i) => (
              <div key={i} className=" flex items-center justify-center">
                <Link
                  to={d.href}
                  className="inline-block text-center py-[6px] px-[13px] text-[#ffffff] my-1 bg-[#5cb85c] border-[#4cae4c] border-[1px] text-[14px]"
                >
                  {d.name}
                </Link>
              </div>
