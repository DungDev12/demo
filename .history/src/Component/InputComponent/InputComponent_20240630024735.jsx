...
const InputComponent = ({ nameI, label, type, valueOut }) => {
  ...
  useEffect(() => {
    if (input === "") {
      ...
      setError("Mã số phải có 10 chữ số");
    } else if (isNaN(input)) {
      setError("Mã số phải là số");
    } else {
      setError("");
    }
  }, [input]);
  return (
    <div>
      <div
        className={`relative flex items-center justify-start gap-2 border-[1px] border-black mt-4 mb-2 p-2 rounded-[4px] group ${
          err != "" ? "border-red-600" : ""
        }`}
      >
        <p
        ...
        <input
          ...
          maxLength={nameI.toLowerCase() === "phone" ?}
        />
      </div>
