...
const Bet = () => {
  ...
  const [valueCheck, setValueCheck] = useState({ tran_id: "", receiver: "" });
  const handleOnChange = (e) => {
    const { name } = e.target;
    const filteredValue = e.target.replace(/[^0-9]/g, "");
    setValue(filteredValue);
    setValueCheck({ ...valueCheck, [name]: e.target });
  };

  const handleCheck = async () => {
    setErr("1");
    try {
      if (valueCheck.receiver != "") {
        const response = await axios.post(
         valueCheck.receiver != "" ?  "https://clmm.bet/api/chuyen-gia-kiem-tra-ma-giao-dich" : ,
          valueCheck
        );
        console.log(response);
      } else {
        const response = await axios.post(
          "https://clmm.bet/api/kiem-tra-ma-giao-dich",
          valueCheck
        );
        console.log(response);
      }
  ...
  const [betPrice, setBetPrice] = useState({ amount: "" });
  const handleOut = (e) => {
    setBetPrice(e);
  };
